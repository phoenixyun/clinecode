...
// ===== Fix 1: Shell Integration stream timeout protection =====
// Max time to wait for the shell integration async iterable to complete.
// If VS Code's Shell Integration API drops the completion marker (]633;D)
// or the stream never closes, the for-await loop would hang forever.
// This timeout forces a break and fallback to terminal capture.
const SHELL_INTEGRATION_STREAM_TIMEOUT_MS = 30_000

// ===== Fix 4: Dynamic fallback timeout =====
// Instead of the old hardcoded 3s, use a slightly longer value that gives
// short commands a better chance to finish. In the future this could be
// made configurable.
const NO_SHELL_INTEGRATION_FALLBACK_TIMEOUT_MS = 5_000

/**
 * Helper: wraps an async iterable with a timeout.
 * If the iterable yields no value within `ms`, we treat it as hung
 * and break out of the loop by rejecting. The caller catches and falls back.
 *
...
export class VscodeTerminalProcess extends EventEmitter<TerminalProcessEvents> implements ITerminalProcess {
	...
	async run(terminal: vscode.Terminal, command: string) {
		...
		// Helper for the final cleanup and emit sequence
		const finishProcess = () => {
			if (this.hotTimer) {
		...
		if (terminal.shellIntegration && terminal.shellIntegration.executeCommand) {
			...
			// ===== Fix 1: Wrap the stream with a timeout =====
			// The `execution.read()` AsyncIterable may never complete if VS Code's
			// Shell Integration fails to send the completion marker. The timeout
			// ensures we eventually break out and fall back to terminal capture.
			let stream: AsyncIterable<string>
			try {
				stream = withTimeout(execution.read(), SHELL_INTEGRATION_STREAM_TIMEOUT_MS)
			} catch {
				// If even creating the timeout wrapper fails, fall back immediately
				Logger.warn("[VscodeTerminalProcess] Failed to create stream timeout wrapper, falling back to terminal capture")
			...
			try {
				...
				// ===== Fix 1 (catch): Stream timed out or errored =====
			...
			// ===== Fix 5: Clean up hotTimer properly =====
			if (this.hotTimer) {
				clearTimeout(this.hotTimer)
				this.hotTimer = null
			}
			...
			// Use the configurable timeout instead of hardcoded 3s
			await new Promise((resolve) => setTimeout(resolve, NO_SHELL_INTEGRATION_FALLBACK_TIMEOUT_MS))
